...
bool MP4Demuxer::open(const std::string& file_path) {
    ...
    // 初始化MP4解复用器（基于验证结果）
    int result = MP4D_open(&demuxer_, readCallback, this, file_.tellg());
    if (result != 1) {
...
bool MP4Demuxer::readNextSample(int track_id, MP4Sample& sample) {
    ...
    // MiniMP4需要手动跟踪当前样本索引
    static unsigned int sample_index = 0;  // 简化版，实际需要为每个track维护
...
bool MP4Demuxer::seekToTime(double timestamp) {
    ...
    if (timestamp == 0.0) {
        // 重新打开文件来重置位置
        // 这不是最优解，但MiniMP4API限制
        current_time_ = 0.0;
        return true;
    }
    
    // 对于非零时间戳，需要逐帧查找（性能较差但功能完整）
